...
| 闸门 | 「允许 Wanna 操作电脑」管点击/输入；**文件访问清单**管文件读写（逐路径、读写分离） |
...
| [`08-组装机制与上下文管理.md`](08-组装机制与上下文管理.md) | **组装的核心设计** | 原料清单、MCP 运行时接入（快路径不碰、组装现场调）、技能位置与两级加载硬保证（永不全量）、组装上下文压缩（任务卡+进度摘要+近窗5步）、坐标吸附、哪些进提示词哪些是管线行为 |
...
| [`09-主agent与三个subagent.md`](09-主agent与三个subagent.md) | **架构定案（2026-09-25）** | 主 agent 的提示词只剩四样（屏幕/用户的话/历史/sub agent 目录）；三个 sub agent（图形/执行/文本）各有权限与工具，结果回主循环做确认，完成后在鼠标右下角给对号+摘要；**标准循环 + 旁路高频直通两条并列的路，不把高频需求混进 agent 分类** |
...
| 组装 / 技能加载 / 上下文压缩 / 坐标吸附 | [`08`](08-组装机制与上下文管理.md) 第八节（A1–A8） |
