...
- **Settings**: one window, seven pages — 通用 / 模型 / 对话与记忆 / 听（识别）/ 说（播报）/ 看与截图 / 快捷键 — see [Settings](#settings). 34 of the 37 settings live in `AppSettings.json`; the other three are the model roles. Each page's sidebar count is derived from the live settings on it, so a decorative row would have to lie about its own size. The one row that is not a setting is 看与截图's 「辅助功能权限」, which reports and requests a macOS permission rather than storing a preference — so it sits outside the count the sidebar shows (8) rather than inflating it.
...
- **Computer Control**: The same 0–1000 grid drives real actions — `[CLICK:]`, `[SCROLL:]`, `[TYPE:]`, `[PRESS:]`, `[OPEN:]`, `[AX_TREE]` — executed through `MacosUseSDK` (SPM, pinned to a revision). See [Acting on the computer](#acting-on-the-computer); the coordinate-space and safety decisions live there.
...
| 看与截图 | 截图清晰度、截图压缩质量、多显示器发送策略、回答里的位置自动飞过去指、单次回答字数上限、允许 Clicky 操作电脑、允许打字和按快捷键、输入方式 | `AppSettings.json` |
...
The model can do more than point — `[CLICK:]`, `[RIGHT_CLICK:]`, `[DOUBLE_CLICK:]`, `[SCROLL:]`, `[TYPE:]`, `[PRESS:]`, `[OPEN:]` and `[AX_TREE]` are executed for real by `MacosUseController`, the **only file that imports `MacosUseSDK`** (SPM, pinned to revision `a2d78663`; the same SDK the machine's `mcp-server-macos-use` project uses). Four things about it are load-bearing:
...
| `AppSettings.swift` | ~475 | Pure data, no I/O: every user-facing setting that is not a model choice, plus `AnswerLengthStyle`, `TranscriptionLanguage`, `CursorPresenceMode`, `CursorShapeStyle`, `CursorFollowDistance`, `TextEntryMethod` and `clamped()`. The 34 stored properties are the 34 rows the settings pages show — the sidebar's per-page counts are derived from the same set. |
...
| `GeneralSettingsView.swift` | ~1101 | The six non-模型 settings pages (通用 / 对话与记忆 / 听 / 说 / 看 / 快捷键) and the components they share — `SettingsRow`, `SettingsCard`, `SettingsSwitch`, `SettingsSlider`, `SettingsStepper`, `SettingsTextEditorRow`, the two pickers. All 34 settings are here; none of them is decorative. |
