...
class Parser {
    tokens;
    position = 0;
    constructor(source) { this.tokens = tokenize(source); }
    get current() { return this.tokens[this.position]; }
    advance() { return this.tokens[this.position++]; }
    accept(kind) {
    ...
    unary() {
        if (this.current.kind === "+" || this.current.kind === "-") {
            ...
            return { type: "unary", operator: token.kind, operand: this.unary(), line: token.line, column: token.column };
        }
        return this.postfix();
    }
    postfix() {
    ...
    primary() {
        ...
        if (this.accept("number"))
            return { ...token, type: "literal", value: token.value, literalKind: "number" };
        if (this.accept("string"))
            return { ...token, type: "literal", value: token.value, literalKind: "string" };
        if (this.accept("identifier")) {
            const name = String(token.value);
            if (functions.has(name) && this.current.kind === "(")
                return this.call(token, name);
            if (name === "true" || name === "false")
                return { ...token, type: "literal", value: name === "true", literalKind: "boolean" };
            if (name === "none")
                return { ...token, type: "literal", value: null, literalKind: "none" };
            if (colors.has(name))
                return { ...token, type: "literal", value: name, literalKind: "color" };
            if (enums.has(name))
                return { ...token, type: "literal", value: name, literalKind: "enum" };
