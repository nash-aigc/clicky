...
export function tokenize(input) {
    ...
    const fail = (message, atLine = line, atColumn = column) => {
    ...
    while (i < source.length) {
        ...
        if (ch === "\r" || ch === "\n") {
            if (ch === "\r" && source[i + 1] === "\n")
                i++;
            if (nesting === 0)
                result.push({ kind: "newline", line, column });
            i++;
            line++;
            column = 1;
            continue;
        ...
        if (ch === '"') {
            ...
            if (i >= source.length)
                fail("字符串未闭合", startLine, startColumn);
            i++;
            column++;
        ...
        if (ch >= "0" && ch <= "9") {
            ...
            if (source[i] === ".") {
                i++;
                if (!isDigit(source[i]))
                    fail("小数点后必须有数字", line, startColumn);
                while (isDigit(source[i]))
                    i++;
            }
            if (source[i] === "e" || source[i] === "E") {
                ...
                while (isDigit(source[i]))
                    i++;
                if (i === exponentStart)
                    fail("指数部分缺少数字", line, startColumn);
            }
            const raw = source.slice(start, i);
            const value = Number(raw);
            if (!Number.isFinite(value))
                fail(`数字 ${raw} 超出有限 Number 范围`, line, startColumn);
        ...
        if (punctuation.has(ch)) {
            result.push({ kind: ch, value: ch, line, column });
            if (ch === "(" || ch === "[")
                nesting++;
            if (ch === ")" || ch === "]") {
                nesting--;
