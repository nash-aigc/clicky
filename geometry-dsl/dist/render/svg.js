import { arcDirection, cross, normalize, regionBounds, sub } from "../geometry/index.js";
import { pathSvgData, samplePath } from "../geometry/curves.js";
import { estimateTextDimensions, layoutLabels } from "./layout.js";
...
function resolveSize(options, xSpan, ySpan, padding) {
    let width;
    let height;
...
function geometryBounds(objects, includeFixedTextAnchors = true) {
    ...
    for (const object of objects) {
        if (!object.style?.visible || object.type === "Mark")
            continue;
        if (object.type === "Point")
            add(object.x, object.y);
        else if (object.type === "Line") {
            add(object.a.x, object.a.y);
            add(object.b.x, object.b.y);
        }
            ...
            if (region) {
                add(region.minX, region.minY);
                add(region.maxX, region.maxY);
            }
        }
        else {
            if (object.position.kind === "fixed") {
                if (includeFixedTextAnchors)
                    add(object.position.x, object.position.y);
            }
            else {
    ...
    if (bounds.maxX - bounds.minX < 1e-12) {
        bounds.minX -= 1;
        bounds.maxX += 1;
    }
    if (bounds.maxY - bounds.minY < 1e-12) {
        bounds.minY -= 1;
        bounds.maxY += 1;
    }
    return bounds;
}
function boundsWithFixedText(objects, options, padding, labelSize) {
    ...
    for (let iteration = 0; iteration < 8; iteration++) {
        ...
        if (Math.max(Math.abs(next.minX - bounds.minX), Math.abs(next.maxX - bounds.maxX), Math.abs(next.minY - bounds.minY), Math.abs(next.maxY - bounds.maxY)) < 1e-9)
            return next;
        bounds = next;
    }
    return bounds;
...
export function renderSvg(scene, options = {}) {
    ...
    const sx = (x) => padding + (x - bounds.minX) * scale;
    const sy = (y) => height - padding - (y - bounds.minY) * scale;
    const wx = (x) => bounds.minX + (x - padding) / scale;
    const wy = (y) => bounds.minY + (height - padding - y) / scale;
    const f = (value) => format(value, precision);
    ...
    const masks = scene.objects
        .filter((object) => object.type === "Region")
    ...
    if (options.background)
        out.push(`<rect width="100%" height="100%" fill="${escapeXml(options.background)}"/>`);
    for (const object of objects) {
        if (object.type === "Point")
            out.push(renderPoint(object, sx, sy, f, labels.get(object.objectId)));
        else if (object.type === "Line")
            out.push(renderLine(object, bounds, sx, sy, f));
        else if (object.type === "Circle")
            out.push(renderCircle(object, sx, sy, scale, f));
        else if (object.type === "Arc")
            out.push(renderArc(object, sx, sy, scale, f));
        else if (object.type === "Path")
            out.push(renderPath(object, sx, sy, f));
        else if (object.type === "Mark")
            out.push(renderMark(object, sx, sy, scale, f));
        else if (object.type === "Region")
            out.push(renderRegion(object, width, height));
        else
            out.push(renderText(object, labels.get(object.objectId), f));
    }
    out.push("</svg>");
    return out.join("\n");
}
function baseStyle(style) {
    ...
    if ("width" in style) {
        ...
        if (style.dashed)
            attrs.push(`stroke-dasharray="${style.width * 6} ${style.width * 4}"`);
    }
    return attrs.join(" ");
}
function renderPoint(point, sx, sy, f, label) {
    const style = point.style, x = sx(point.x), y = sy(point.y), size = style.size;
    const parts = [`<g data-type="Point" data-object-id="${point.objectId}" opacity="${style.opacity}" fill="${style.color}" stroke="${style.color}">`];
    if (style.shape === "dot")
        parts.push(`<circle cx="${f(x)}" cy="${f(y)}" r="${f(size / 2)}"/>`);
    else if (style.shape === "circle")
        parts.push(`<circle cx="${f(x)}" cy="${f(y)}" r="${f(size / 2)}" fill="none" stroke-width="1.5"/>`);
    else if (style.shape === "square")
        parts.push(`<rect x="${f(x - size / 2)}" y="${f(y - size / 2)}" width="${f(size)}" height="${f(size)}"/>`);
    else
        parts.push(`<path d="M ${f(x - size / 2)} ${f(y - size / 2)} L ${f(x + size / 2)} ${f(y + size / 2)} M ${f(x - size / 2)} ${f(y + size / 2)} L ${f(x + size / 2)} ${f(y - size / 2)}" fill="none" stroke-width="1.5"/>`);
    if (label) {
...
function renderRegionMask(region, width, height, sx, sy, scale, f) {
    const expression = region.expression;
...
function renderRegion(region, width, height) {
    if (region.style.fill === null || region.style.opacity <= 0)
        return "";
...
function renderText(text, label, f) {
    if (!label)
        return "";
...
function renderLine(line, bounds, sx, sy, f) {
...
function renderCircle(circle, sx, sy, scale, f) {
...
function renderArc(arc, sx, sy, scale, f) {
...
function renderPath(path, sx, sy, f) {
...
function renderMark(mark, sx, sy, scale, f) {
    const style = mark.style, paths = [];
    const point = (p) => ({ x: sx(p.x), y: sy(p.y) });
    if (mark.kind === "right") {
        const [a, b, c] = mark.operands, pa = point(a), pb = point(b), pc = point(c);
        ...
        for (const line of mark.operands) {
            const a = point(line.a), b = point(line.b), mid = { x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 };
            const d = normalize(sub(b, a)), n = { x: -d.y, y: d.x }, s = style.size;
            if (mark.kind === "equal")
                paths.push(`M ${f(mid.x - n.x * s)} ${f(mid.y - n.y * s)} L ${f(mid.x + n.x * s)} ${f(mid.y + n.y * s)}`);
            else
                paths.push(`M ${f(mid.x - d.x * s - n.x * s)} ${f(mid.y - d.y * s - n.y * s)} L ${f(mid.x + n.x * s)} ${f(mid.y + n.y * s)} L ${f(mid.x + d.x * s - n.x * s)} ${f(mid.y + d.y * s - n.y * s)}`);
        }
    }
    else {
        const p = mark.operands;
        const angleSets = [[p[0], p[1], p[2]], [p[3], p[4], p[5]]];
