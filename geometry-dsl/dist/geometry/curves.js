...
export function pathPoint(path, segment, u) {
    ...
    const t = t1 + u * (t2 - t1);
    const blend = (a, b, ta, tb) => add(mul(a, (tb - t) / (tb - ta)), mul(b, (t - ta) / (tb - ta)));
...
export function samplePath(path, smoothSteps = 96) {
...
export function pathSvgData(path, sx, sy) {
    if (!path.smooth) {
    ...
    for (const sample of samples.slice(1))
        commands.push(`L ${sx(sample.point.x)} ${sy(sample.point.y)}`);
    if (path.closed)
        commands.push("Z");
    return commands.join(" ");
}
export function pointFromVec(point) {
