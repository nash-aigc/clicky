import { samplePath } from "./curves.js";
...
function intersectionBounds(bounds) {
    if (!bounds.length)
        return null;
    const result = {
...
export function regionBounds(region) {
    ...
    if (expression.kind === "inside")
        return sourceBounds(expression.source);
    if (expression.kind === "difference")
        return regionBounds(expression.left);
    const operandBounds = expression.operands.map(regionBounds);
    if (expression.kind === "intersection" && operandBounds.some(bound => bound === null))
        return null;
    const bounds = operandBounds.filter((bound) => bound !== null);
...
function pathWinding(point, vertices) {
...
function pathSignedDistance(path, point) {
    const vertices = pathVertices(path);
    let boundaryDistance = Infinity;
    for (let index = 0; index < vertices.length; index++) {
        boundaryDistance = Math.min(boundaryDistance, segmentDistance(point, vertices[index], vertices[(index + 1) % vertices.length]));
    }
...
export function regionSignedDistance(region, point) {
    const expression = region.expression;
    if (expression.kind === "inside") {
    ...
    if (expression.kind === "difference") {
        return Math.min(regionSignedDistance(expression.left, point), -regionSignedDistance(expression.right, point));
    }
...
export function regionContains(region, point, tolerance = 0) {
