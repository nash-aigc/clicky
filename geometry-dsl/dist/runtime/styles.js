...
const common = { visible: true, color: "#000000", opacity: 1, layer: 0 };
const stroke = { ...common, width: 1, dashed: false };
export function defaultStyle(type) {
    if (type === "Point")
        return { ...common, size: 4, shape: "dot", label: "auto", label_pos: "auto" };
    if (type === "Line")
        return { ...stroke, arrow: null };
    if (type === "Circle")
        return { ...stroke, fill: null };
    if (type === "Arc")
        return { ...stroke };
    if (type === "Path")
        return { ...stroke, fill: null, fill_rule: "evenodd" };
    if (type === "Text")
        return { ...common, size: null };
    if (type === "Region") {
...
export function normalizeColor(value, key) {
    if (value === null && key === "fill")
        return null;
    if (typeof value !== "string")
        throw new Error(`${key} 必须是颜色${key === "fill" ? "或 none" : ""}`);
...
export function applyStyles(type, base, overrides) {
    const result = { ...base };
    for (const [key, value] of overrides) {
        if (!styleKeys.has(key))
            throw new Error(`未知样式参数 ${key}`);
        if (!applicable[type].has(key))
            throw new Error(`样式参数 ${key} 不适用于 ${type}`);
        if (key === "color" || key === "fill") {
            result[key] = normalizeColor(value, key);
            continue;
        }
        if (key === "visible" || key === "dashed") {
            ...
            if (typeof value !== "number" || !Number.isFinite(value) || value <= 0)
                throw new Error(`${key} 必须是大于 0 的 Number`);
        }
        else if (key === "opacity") {
            if (typeof value !== "number" || value < 0 || value > 1)
                throw new Error("opacity 必须位于 [0, 1]");
        }
        else if (key === "layer") {
            if (typeof value !== "number" || !Number.isInteger(value))
                throw new Error("layer 必须是整数");
        }
        else if (key === "label") {
            if (!(value === null || value === "auto" || typeof value === "string"))
                throw new Error("label 必须是 auto、none 或 String");
        }
        else if (!enumValues[key]?.has(value))
            throw new Error(`${key} 使用了非法枚举值`);
        result[key] = value;
    }
    return result;
}
