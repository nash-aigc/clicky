...
export declare class GeometryDslError extends Error {
    readonly code: string;
    readonly location?: SourceLocation;
    constructor(code: string, message: string, location?: SourceLocation);
}
export type TokenKind = "number" | "string" | "identifier" | "newline" | "eof" | "(" | ")" | "[" | "]" | "," | "=" | "." | "+" | "-" | "*" | "/";
...
export type PointStyle = CommonStyle & {
    ...
    label_pos: "auto" | "above" | "below" | "left" | "right" | "above_left" | "above_right" | "below_left" | "below_right";
};
export type LineStyle = StrokeStyle & {
    arrow: "start" | "end" | "both" | null;
};
export type CircleStyle = StrokeStyle & {
    fill: Color | null;
};
export type PathStyle = CircleStyle & {
    fill_rule: "evenodd" | "nonzero";
};
export type MarkStyle = StrokeStyle & {
    size: number;
};
...
export type RegionExpression = {
    kind: "inside";
    source: CircleValue | PathValue;
} | {
    kind: "union" | "intersection";
    operands: RegionValue[];
} | {
    kind: "difference";
    left: RegionValue;
    right: RegionValue;
...
export type MarkValue = ValueBase & {
    type: "Mark";
    kind: "right" | "equal" | "equal_angle" | "parallel";
    operands: GeometryValue[];
    style: MarkStyle;
};
export type GeometryValue = PointValue | LineValue | CircleValue | ArcValue | PathValue | MarkValue | TextValue | RegionValue;
