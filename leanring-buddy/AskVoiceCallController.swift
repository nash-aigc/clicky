...
final class AskVoiceCallController: ObservableObject {
    ...
    ///
    /// 同时是 Ask 页语音通话那张卡片底部要显示的**时间**：与打字那条路的
    /// `CompanionManager.currentReplyReceivedAt` 同一个含义（用户 2026-09-25：
    /// 「缺少时间参数……应该显示在界面上」）。
    private(set) var assistantTextBeganAt: Date?
    ...
    /// **正常速度：25 字/秒。**
    ///
    /// 这个数**不是**从别处抄来的，是按"快的那一段要撑得住"反算出来的。
    ///
    /// 一度设成 83（实测 Ask 页 DeepSeek 那条路的速度），但那个数**在这里做不到**：
    /// 83 字/秒会在 0.7 秒内把 60 字缓冲耗光，然后猛地掉到最低速 —— 那 0.7 秒的
    /// "快"既没让用户看清什么，又把落差拉得更陡。用户 2026-09-25 明确取消了这条
    /// 约定：「83 个字可能完全无法实现……可以把正常速度降低一点，这样体验会稍微
    /// 流畅一些」。
    ///
    /// 均衡点由物理决定、改不了：交付多少就得吐多少（实测交付约 8 字/秒），
    /// 所以**能设计的只有曲线的形状**。25 字/秒意味着 60 字缓冲能撑
    ...
    /// 慢慢回补、把速率重新顶上去；取在上面就等于永远追不上，一路滑到空。
    /// 同时它也不是"停住"——只是慢。
    private static let displayMinimumCharactersPerSecond = 6.0
    ...
    /// ## 四个数是怎么算出来的
    ///
    /// 一个**实测**输入决定一切：全双工的交付是 **约 8 字/秒，而且是成串的** ——
    /// 一整句几乎同时生成完，然后音频慢慢播，中间是静默。
    ///
    /// 由此可以定死两件事：
    ///
    /// 1. **平均吐字速度不可能超过交付速度。** 缓冲只能重新分配时间，不能造字。
    ///    所以"全程都快"是空的，能设计的只有**曲线的形状**：开头快、之后平滑降下来。
    /// 2. 要让开头那几行真的跑起来，就得**先攒够那几行**。一行约 27~33 字
    ///    （实测：断行器在 459pt 宽下每行 33 个 unit），用户要的是「前面两行一定
    ///    正常速度」→ **阈值 60 字**，它同时就是"快节奏所需的在手缓冲"：一开口就是
    ///    快的，然后随着缓冲变薄而**按比例**降速。
    ///
    /// 速率式（连续，所以不会有"突然减速"）：
    ///
