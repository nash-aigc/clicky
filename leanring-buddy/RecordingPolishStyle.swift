...
nonisolated final class RecordingPolishStyleStore {
    ...
    static let didChangeNotification = Notification.Name("clickyRecordingPolishStylesDidChange")
...
【屏幕模式 · 追加规则（同样不属于原始提示词）】

在开始整理之前，**先检查用户提示词的开头或末尾**是否出现与屏幕有关的词：「屏幕」「看一下屏幕」「屏幕里」「屏幕上」「看我的屏幕」等。

- **出现了** → 用户是在问屏幕上的内容。**必须结合随附的屏幕截图回答**（见消息里的 `<screenshot>` 块），答案以图为准；图里确实没有就直说看不到，不要编。此时不要把它整理成清单，直接回答问题。
- **没出现** → 按上面的要求正常整理。
"""
}
