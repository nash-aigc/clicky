...
struct GeneralSettingsView: View {
    ...
    private var listenPage: some View {
        Group {
            ...
            // **这一行不是开关，是说明。** 用户 2026-09-25 明确要求：
            // 「我觉得这个东西一定要在设置页面说清楚，否则未来用户可能找不到原因」。
            //
            // 事情本身：全双工语音通话会**额外**跑一路识别，只为把用户说的话立刻
            // 显示出来（全双工协议要等一轮答完才下发转写）。它用的是**这一页选的那个
            // 识别角色** —— 所以用户在这里改模型，会同时影响通话里"你说的话"的显示速度
            // 和准确率。不说清楚的话，出了问题没人会想到这里。
            SettingsGroupLabel("语音通话")
            SettingsCard {
                SettingsRow(
                    label: "全双工语音通话也会用它",
                    description: "打全双工语音电话时，这里选的识别模型会**再跑一路**，专门用来把你说的话立刻显示在对话里。"
                        + "原因是全双工协议要等 AI 把一整句话答完才下发你的转写 —— 不另跑这一路，你说的内容就要等到回答结束才出现。"
                        + "这一路只负责显示：真正记进对话的仍是对面发回来的转写。"
                        + "所以在这里换模型，会同时改变通话中「你说的话」显示得多快、多准。"
                )
            }
